...
def statistics_view(request):
    ...
    for wt in workout_types:
        ...
        if wt.category == 'group':
            ...
            if sessions_sum:
                profit += sessions_sum
        workout_profit[wt.name] = float(profit) if profit else 0
        
    sorted_profitability = sorted(workout_profit.items(), key=lambda x: x[1], reverse=True)

    # 5. Clients alphabetical payment lists
    client_payment_list = []
    ...
    for cl in all_clients:
        ...
        total_spent = float(grp_sum) + float(ind_sum) + float(card_sum)
        client_payment_list.append({
            ...
            'total_paid': total_spent
        })

    # 6. Revenue grouped by month (Simulated)
    revenue_chart_data = [
